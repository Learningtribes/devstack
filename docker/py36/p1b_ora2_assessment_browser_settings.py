# -*- coding: utf-8 -*-
"""Isolated non-eager LMS/worker settings for the P1-B ORA2 gate."""
...
ORA2_GATE_RUNTIME = _required('PY36_R1_ORA2_RUNTIME')
ORA2_GATE_NAMESPACE = _required('PY36_R1_ORA2_NAMESPACE')
SITE_NAME = _required('PY36_R1_ORA2_SITE_DOMAIN')
SQL_DATABASE = _required('PY36_R1_ORA2_SQL_DATABASE')
SQL_HISTORY_DATABASE = _required('PY36_R1_ORA2_SQL_HISTORY_DATABASE')
SQL_USER = _required('PY36_R1_ORA2_SQL_USER')
SQL_PASSWORD = _required('PY36_R1_ORA2_SQL_PASSWORD')
MONGO_MODULESTORE_DATABASE = _required('PY36_R1_ORA2_MONGO_MODULESTORE_DATABASE')
MONGO_CONTENTSTORE_DATABASE = _required('PY36_R1_ORA2_MONGO_CONTENTSTORE_DATABASE')
MONGO_USER = _required('PY36_R1_ORA2_MONGO_USER')
MONGO_PASSWORD = _required('PY36_R1_ORA2_MONGO_PASSWORD')
MONGO_CONTAINER = _required('PY36_R1_ORA2_MONGO_CONTAINER')
RABBIT_CONTAINER = _required('PY36_R1_ORA2_RABBIT_CONTAINER')
RABBIT_USER = _required('PY36_R1_ORA2_RABBIT_USER')
RABBIT_PASSWORD = _required('PY36_R1_ORA2_RABBIT_PASSWORD')
CACHE_CONTAINER = _required('PY36_R1_ORA2_CACHE_CONTAINER')
...
STATIC_ROOT = os.path.join('/tmp', ORA2_GATE_NAMESPACE, 'staticfiles-output')
...
for alias, cache in CACHES.items():
    ...
    if backend.endswith('LocMemCache'):
        cache['LOCATION'] = '{}:{}'.format(ORA2_GATE_NAMESPACE, alias)
    else:
        cache['LOCATION'] = ['{}:11211'.format(CACHE_CONTAINER)]
    cache['KEY_PREFIX'] = '{}_{}'.format(ORA2_GATE_NAMESPACE, cache.get('KEY_PREFIX', alias))

RABBIT_VHOST = ORA2_GATE_NAMESPACE
...
CELERY_DEFAULT_EXCHANGE = '{}.edx.core'.format(ORA2_GATE_NAMESPACE)
CELERY_DEFAULT_EXCHANGE_TYPE = 'direct'
CELERY_DEFAULT_QUEUE = '{}.default'.format(ORA2_GATE_NAMESPACE)
CELERY_DEFAULT_ROUTING_KEY = CELERY_DEFAULT_QUEUE
CELERY_QUEUE_HA_POLICY = 'all'
CELERY_CREATE_MISSING_QUEUES = False

ORA2_GATE_QUEUES = {
    'default': '{}.default'.format(ORA2_GATE_NAMESPACE),
    'grade': '{}.grade'.format(ORA2_GATE_NAMESPACE),
    'progress': '{}.progress'.format(ORA2_GATE_NAMESPACE),
}
CELERY_QUEUES = {
    queue: {
    ...
    for queue in ORA2_GATE_QUEUES.values()
}
RECALCULATE_GRADES_ROUTING_KEY = ORA2_GATE_QUEUES['grade']
RECALCULATE_PROGRESS_ROUTING_KEY = ORA2_GATE_QUEUES['progress']
POLICY_CHANGE_GRADES_ROUTING_KEY = ORA2_GATE_QUEUES['grade']

# -*- coding: utf-8 -*-
"""Run the isolated ORA2 grade/progress worker with a stable sync loop."""
...
def ora2_sync_loop(obj, connection, consumer, blueprint, hub, qos, heartbeat, clock,
                   hbrate=2.0, **kwargs):
    """Keep the legacy py-amqp connection alive while consuming with a timeout."""
...
def main():
    ...
    if missing:
        raise RuntimeError('missing required ORA2 downstream tasks: {}'.format(missing))

    controller = WorkController(
        app=APP,
        hostname='{}@%h'.format(os.environ.get(
            'PY36_R1_ORA2_WORKER_HOSTNAME', 'p1b-ora2-assessment-worker'
        )),
        loglevel='INFO',
        pool='solo',
        concurrency=1,
        use_eventloop=False,
        queues=(settings.ORA2_GATE_QUEUES['grade'], settings.ORA2_GATE_QUEUES['progress']),
        without_mingle=True,
        without_gossip=True,
        without_heartbeat=False,
    )
    controller.consumer.amqheartbeat = settings.BROKER_HEARTBEAT
    controller.consumer.loop = ora2_sync_loop
    sys.stdout.write(
        'P1B_ORA2_WORKER_READY queues={} required={} broker_heartbeat={} loop=sync\n'.format(
            settings.ORA2_GATE_QUEUES,
            required,
